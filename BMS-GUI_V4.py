...
class BatteryManagementSystem:
    ...
    def create_voltages_tab(self):
        ...
        self.v_scrollbar = ttk.Scrollbar(
            self.voltage_view_frame, orient=VERTICAL, command=self.v_canvas.yview)
        self.v_scrollbar.pack(side=RIGHT, fill=Y)
        self.v_canvas.configure(yscrollcommand=self.v_scrollbar.set)

        # Horizontal scrollbar
        self.v_hscrollbar = ttk.Scrollbar(
            self.voltage_view_frame, orient=HORIZONTAL, command=self.v_canvas.xview)
    ...
    def create_dynamic_widgets(self, stack_rows, stack_cols, cells, temps):
        ...
        self.data_frame = ttk.LabelFrame(
            self.overview_frame, text="Data", padding=(10, 5))
        ...
        self.plot_frame = ttk.LabelFrame(
            self.overview_frame, text='Plot', padding=(10, 5))
        self.plot_frame.grid(row=0, column=1,
                             padx=10, pady=5)
        # SoC
        self.SoC_button = ttk.Button(
            self.plot_frame, text='SoC', command=lambda: plot_data(
                timestamps, SoC, 'Time (s)', 'State of Charge (%)', 'State of Charge'))
        self.SoC_button.grid(row=0, column=0, padx=10, pady=5)
        # VsBat
        self.VsBat_button = ttk.Button(
            self.plot_frame, text='VsBat', command=lambda: plot_data(
                timestamps, VsBat, 'Time (s)', 'VsBat (V)', 'VsBat'))
        self.VsBat_button.grid(row=1, column=0, padx=10, pady=5)
        # VsHV
        self.VsHV_button = ttk.Button(
            self.plot_frame, text='VsHV', command=lambda: plot_data(
                timestamps, VsHV, 'Time (s)', 'VsHV (V)', 'VsHV'))
        self.VsHV_button.grid(row=2, column=0, padx=10, pady=5)
        # Current
        self.curr_button = ttk.Button(
            self.plot_frame, text='Current', command=lambda: plot_data(
                timestamps, curr, 'Time (s)', 'Current (A)', 'Current'))
        self.curr_button.grid(row=3, column=0, padx=10, pady=5)
